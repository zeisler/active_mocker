    ...
    module Scopes
      ...
      def scope_methods
        class_introspector.class_macros.select { |h| h.keys.first == :scope }.map do |h|
          ...
          body       = if safe_methods[:scopes].include?(name)
                         find_scope_body(name)
                       else
      ...
      def ast_scopes
        @ast_scopes ||= class_introspector.parsed_source.class_begin.children.select { |n| n.try(:type) == :send && n.try{children[1] == :scope} }
      end

      def find_scope_body(name)
