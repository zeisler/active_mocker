RSpec.describe "Gemfile Version Match" do
  it "rails_4.0.gemfile.lock" do
    file = File.join(File.expand_path('../../', __FILE__), "test_rails_4_app/gemfiles/rails_4.0.gemfile.lock")
    expect(File.readlines(file).grep(/active_mocker \(#{Regexp.quote(ActiveMocker::VERSION)}\)/).size > 0).to eq true
  end
